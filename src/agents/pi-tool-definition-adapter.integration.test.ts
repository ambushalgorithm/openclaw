...
function createMockTool(name: string, executeResult: unknown) {
  return {
    name,
    label: name,
...
describe("toToolDefinitions - Integration", () => {
  it("wraps tool execution with timing", async () => {
    ...
    const definitions = toToolDefinitions([mockTool as any]);
  ...
  it("preserves error results", async () => {
    ...
    const definitions = toToolDefinitions([errorTool as any]);
    ...
    const contentText = JSON.stringify((result.content as any[])[0]?.text || "");
  ...
  it("passes hook context through execution", async () => {
    ...
    const definitions = toToolDefinitions([mockTool as any], hookContext);
    const result = await definitions[0].execute(
      "call-1",
      { param: "value" },
      undefined,
  ...
  it("handles abort signal correctly", async () => {
    ...
    const definitions = toToolDefinitions([abortingTool as any]);
  ...
  it("handles update callbacks", async () => {
    ...
    const definitions = toToolDefinitions([updatingTool as any]);
    const result = await definitions[0].execute(
      "call-1",
      {},
  ...
  it("passes correct parameters to tool", async () => {
    ...
    const definitions = toToolDefinitions([paramTool as any]);
  ...
  it("handles multiple tools independently", async () => {
    ...
    const definitions = toToolDefinitions([toolA as any, toolB as any]);
